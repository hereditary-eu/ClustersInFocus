    ...
    try:
        ...
        if not raw_data:
            ...
            if not request.data:
                raise HTTPException(
                    status_code=400, 
                    detail="Dataset not found in the backend and no data provided in the request"
                )
        ...
        try:
            ...
            if not dataset_id:
                filename = request.filename if hasattr(request, 'filename') and request.filename else "dataset_from_computation.csv"
