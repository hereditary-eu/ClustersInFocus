    ...
    try:
        ...
        try:
            ...
            if not request.dataset_id or request.data:
                filename = request.filename if hasattr(request, 'filename') and request.filename else "dataset_from_shapley.csv"
...
async def get_shapley_values_endpoint(dataset_id: str, target_column: str, db: Session = Depends(get_db)) -> List[Dict[str, Any]]:
    try:
        values = get_shapley_values(db, dataset_id, target_column)
        if not values:
            raise HTTPException(
                status_code=404, 
                detail=f"Shapley values for target column {target_column} not found"
            )
